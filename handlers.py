...
def setup_import_alias(alias, path):
    """
        Сей костыль позволяет функциям, входящим в состав пакета с правилами, получать доступ к другим функциям
        в этом пакете по абсолютному имени. Структура правил в raava устроена так, что каждый из пакетов не имеет
        фиксированного имени корня, я именуется с суффиксом версии гита, например. Иными словами, если у нас есть
        репозиторий с правилами, названный "rules", то в локальный каталог их версии будут складываться под
        именами вида:
            - git_123
            - git_456
            - git_789
        Каждый из них будет содержать полное дерево питонового кода какой-то конкретной ревизии. Когда raava
        ищет правило, то читает рядом с этими каталогами симлинку HEAD, указывающую на каталог с последней
        ревизией. Далее, этими каталогами система пользуется, как обычными пакетами и импортирует их в
        sys.modules именно под именами "git_xxx". Логично, что функции внутри пакета не могут знать о том, как
        называется корень пакета, поэтому был придуман этот хак с setup_import_alias(). Суть его в том, чтобы
        предоставить возможность использования абсолютного импорта (import rules, from rules import xxx), вместо
        того, чтобы вынуждать пользователя считать точки (from ..... import xxx).

        Для понимания работы кода ниже, советую ознакомиться с http://legacy.python.org/dev/peps/pep-0302/
        ...
        вверх по стеку и находит первый же модуль, в названии которого встречается path (полный путь к каталогу с
        версиями), затем вычленяет из него название пакета git_xxx и импортирует его.
